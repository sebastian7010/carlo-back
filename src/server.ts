...
// POST /prayers -> crear
app.post("/prayers", async (req: Request, res: Response) => {
  try {
    const { title, content, category, approved } = req.body ?? {};
...
app.patch("/prayers/:id", async (req: Request, res: Response) => {
  try {
...
app.delete("/prayers/:id", async (req: Request, res: Response) => {
  try {
    const id = String(req.params.id || "");
